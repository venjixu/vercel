...
export const PaymentModal: React.FC<{
  ...
  const pay = async () => {
    ...
    setError(null);
    try {
      if (!supabase) throw new Error('Payment login is not configured.');
      const { data: { session } } = await supabase.auth.getSession();
      if (!session) throw new Error('Please sign in before purchasing a premium plan.');
      ...
      const orderResponse = await fetch('/api/payments/create-order', {
        method: 'POST',
        headers: authHeaders,
        body: JSON.stringify({ planId, accessToken: session.access_token }),
      });
          ...
          handler: async (response: Record<string, string>) => {
            try {
              const verifyResponse = await fetch('/api/payments/verify', {
                method: 'POST',
                headers: authHeaders,
                body: JSON.stringify({ planId: plan, ...response, accessToken: session.access_token }),
              });
