...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const auth = String(req.body?.accessToken || req.query?.accessToken || '').replace(/^Bearer\s+/i, '');
  ...
  try {
    ...
    if (action === 'verify') {
      ...
      if (order.notes?.userId && order.notes.userId !== (user?.id || (req as any).fresherUser.id)) {
      ...
      await saveEntitlement(user.id, entitlement);
